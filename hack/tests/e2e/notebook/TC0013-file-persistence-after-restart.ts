...
/**
 * TC0013 — Files written to /home/{username} survive a notebook restart.
 *
 * The admin pod mounts the NFS-backed per-user subpath at /data/home
 * with a symlink /home/{username} -> /data/home/{username},
...
test.describe('TC0013 重启后用户文件持久化', () => {
  ...
  test('TC0013a: /home/admin 文件在重启后保留', async ({ adminPage }) => {
    ...
    // ── Step 3: Write a sentinel file to /home/admin ──
    const filename = `persist_check_${Date.now()}.txt`
    const writeResult = execInPod(
      'jupyterlab-admin',
      `echo "persistence_test" > /home/admin/${filename} && cat /home/admin/${filename}`,
    )
    ...
    const readResult = execInPod(
      'jupyterlab-admin',
      `cat /home/admin/${filename}`,
    )
    expect(readResult).toContain('persistence_test')

    // ── Cleanup ──
    execInPod('jupyterlab-admin', `rm -f /home/admin/${filename}`)
    await nb.goto()
