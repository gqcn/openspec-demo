...
- [x] **FB-39**：个人信息页面角色名称显示为英文（admin/user），应与用户管理页面一致显示中文（管理员/普通用户）
  - 修复：`profile/index.vue` roles 计算属性返回值从 `['admin']`/`['user']` 改为 `['管理员']`/`['普通用户']`
  - 测试：TC0022c（角色标签断言更新为中文"管理员"）
- [x] **FB-40**：个人信息页面移除多余的诗句文字"领略盛夏日的光，半入江风半入云。"
  - 修复：`profile/index.vue` 删除诗句 `<div>` 块
  - 测试：TC0022c（新增断言确认诗句文字不存在）
